...
class ModelExtensionShippingXdFlat extends Model
{
    ...
    public function replaceRates($rates)
    {
        ...
        foreach ($rates as $rate) {
            ...
            $image = isset($rate['image']) ? trim($rate['image']) : '';
    ...
    {
        if (!$this->columnExists('image')) {
            $this->db->query("ALTER TABLE `" . DB_PREFIX . "xd_flat_rate` ADD `image` VARCHAR(255) NOT NULL DEFAULT '' AFTER `title`");
        }
    }
    ...
    {
        $query = $this->db->query("SHOW COLUMNS FROM `" . DB_PREFIX . "xd_flat_rate` LIKE '" . $this->db->escape($column) . "'");
